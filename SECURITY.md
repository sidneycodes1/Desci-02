...
- **Memo Length Caps**: All expense proposals enforce a strict 256-byte maximum memo limit (`MAX_MEMO_BYTES`) to prevent gas-exhaustion payload attacks.
...
- **PostgreSQL RLS Policies**: Every database table (`projects`, `research_logs`, `expenses`, `milestones`, `treasury_balances`) enforces strict RLS policies.
