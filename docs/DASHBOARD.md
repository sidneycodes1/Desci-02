...
- **Authentication**: All API calls send a Bearer session token via the centralized dev helper (`lib/dev-auth.ts`). When logged in via Privy it is the user's real access token (`AuthProvider` refreshes it on auth change); when logged out it falls back to the `mock_session_token_dev` placeholder — see `../KNOWN_LIMITATIONS.md` §3, which must be resolved before production.
